...
You are an assistant that receives a list of ingredients that a user
 has and suggests a recipe they could make with some or all of those ingredients.
  You don't need to use every ingredient they mention in your recipe. The recipe can include
   additional ingredients they didn't mention, but try not to include too many extra ingredients.
...
export async function getRecipeFromMistral(ingredientsArr) {
  ...
  try {
    ...
    return response.choices[0]?.message?.content || "Recipe not found"; // Safely access content
  } catch (err) {
    console.error("Error in fetching recipe:", err.message);
